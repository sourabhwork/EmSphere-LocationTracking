//
//  MarkAttedanceMapViewController.swift
//  emSphere
//
//  Created by Dhiraj Chaudhari on 23/06/18.
...
import GoogleMaps
import MapKit
class MarkAttedanceMapViewController: UIViewController
{
    //MARK:- Properties
    var timer: Timer?
    ...
    override func viewDidLoad() {
        ...
        lastPunchLabel.text = ""
        lastPunchLabel.isHidden = true //MARK:code 22-03-2021 removve last punc feature
        locationManager.startUpdatingLocation()
        
        self.lastPunchLabel.backgroundColor = #colorLiteral(red: 1.0, green: 1.0, blue: 1.0, alpha: 1.0)
        ...
        NotificationCenter.default.removeObserver(self, name: Notification.Name(rawValue: "GetLocationMap"), object: nil)
        NotificationCenter.default.addObserver(self,selector: #selector(self.showCurrentLocationOnMapview),name: Notification.Name(rawValue: "GetLocationMap"),object: nil)
    ...
    func setupMapView() {
        ...
        let camera = GMSCameraPosition.camera(withLatitude: 18.490877000, longitude: 73.8199829999, zoom: 14.0)
        ...
        self.backgroundMapView.addSubview(mapView!)
    ...
    func setLastPunchAttribuedString(date:String,inOutStatus:String) {
        ...
        let textColor = #colorLiteral(red: 0.3745557666, green: 0.3901350498, blue: 0.4719911814, alpha: 1)
    ...
    @objc func showCurrentLocationOnMapview() {
        ...
        print("gps coordinate =",locationManager.lastLocation!.coordinate)
        let camera = GMSCameraPosition.camera(withLatitude: (locationManager.lastLocation!.coordinate.latitude), longitude: (locationManager.lastLocation!.coordinate.longitude), zoom: 25.0)
        
        self.marker = GMSMarker(position: locationManager.lastLocation!.coordinate)
        self.marker.map = self.mapView
        //update marker to top view
        let latestCordinate = locationManager.lastLocation?.coordinate
        
        if latestCordinate != nil {
            var point = mapView!.projection.point(for: latestCordinate!)
            if (UIDevice.current.userInterfaceIdiom == .pad){
                point.y = point.y + 300
            } else {
                point.y = point.y + 220
            ...
            let projectionCordinate = mapView!.projection.coordinate(for: point)
            let cameraUpdate  = GMSCameraUpdate.setTarget(projectionCordinate)
    ...
    func removePreviousControler() {
        if var viewControllers = self.navigationController?.viewControllers {
            for vc in viewControllers {
                if vc.isKind(of: MarkAttandanceViewController.classForCoder()) {
                    print("It is in stack")
                    //Your Process
                    let index = viewControllers.index(of: vc)
                    print("Index of ViewController Camera Vc =",index)
    ...
    @objc func syncData() {
        ...
        APIManager.getSyncMobileApplicationConfiguration { (result) in
            ...
            if (result){
                ...
                if(UserProfile.getIshideMarkAttendance() == "0") {
                    print("Enter in Camera condn")
                    let markAttandance1 = UIStoryboard.Home().instantiateViewController(withIdentifier: "MarkAttandanceViewController") as! MarkAttandanceViewController
    ...
    func insertOfflineAttendanceMarkData(remark: String,parameter:[String:String],isOffline:Bool) -> Bool{
        ...
        let attedanceInfo = self.getInfoObjectFromDictionary(parameterDic: parameters)
        var status = ""
        if (door == "1") {
            status = "IN"
        } else{
    ...
    func didSyncOfflineIndiual(){
        if let markAtt = offlineMarkAttendanceArray.firstObject as? MarkAttendanceInfo{
            //MARK:code on 04-03-2021
            if markAtt.locationAddress == ""{
                self.getAddressforOfflineAttence(markObj: markAtt, currentAdd: { (adressStr) in
                    markAtt.locationAddress = adressStr
    ...
    func getAddress(currentAdd : @escaping ( _ returnAddress :String)->Void){
        ...
        geocoder.reverseGeocodeCoordinate(coordinate) { response , error in
            if ((error) != nil) {
                ...
                self.view.dissmissLoading()
                self.showAlert(message: "Sorry, unable to address")
                return
            }
    ...
    func getAddressforOfflineAttence(markObj:MarkAttendanceInfo,currentAdd : @escaping ( _ returnAddress :String)->Void){
        
        let lat = Double(markObj.latitude) ?? 0
        let long =  Double(markObj.longitude) ?? 0
        var currentAddress = String()
        if lat == 0 && long == 0{
            currentAdd(currentAddress)
            return
...
extension MarkAttedanceMapViewController: remarkDelegate {
...
extension MarkAttedanceMapViewController: UITableViewDataSource, UITableViewDelegate {
...
extension MarkAttedanceMapViewController: GMSMapViewDelegate {
